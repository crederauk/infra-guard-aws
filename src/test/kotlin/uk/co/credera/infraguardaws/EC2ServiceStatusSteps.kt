...
class EC2ServiceStatusSteps(@Autowired val ssmService: SsmService) :
    ...
    init {
        When(
                "service status on host {string} is active for service {string} with timeout {long} seconds"
        ) { host: String, serviceName: String, timeoutSeconds: Long ->
        ...
        Then("the service is running") { assertThat(commandResult.output).doesNotContain("failed") }
    }
}